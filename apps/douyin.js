...
const DY_API_URL = "https://www.douyin.com/aweme/v1/web/aweme/detail/?device_platform=webapp&aid=6383&channel=channel_pc_web&aweme_id={}&pc_client_type=1&version_code=190500&version_name=19.5.0&cookie_enabled=true";
const COMMON_HEADER = {
...
class SM3 {
    ...
    write(e) {
        const a = typeof e === 'string' ? new TextEncoder().encode(e) : e;
        this.size += a.length;
        let f = 64 - this.chunk.length;
        if (a.length < f) this.chunk = this.chunk.concat(Array.from(a));
        else {
            this.chunk = this.chunk.concat(Array.from(a.slice(0, f)));
            while (this.chunk.length >= 64) {
                this._compress(this.chunk);
                this.chunk = a.length > f ? Array.from(a.slice(f, Math.min(f + 64, a.length))) : [];
                f += 64;
            }
        }
    }
...
function generate_a_bogus(url_search_params, user_agent) {
    ...
    [b[30], b[31]] = [(args[1] / 256) & 255, args[1] % 256 & 255];
    ...
    b[65] = window_env_list.length & 255;
    let checksum = 44 ^ b[20] ^ b[26] ^ b[30] ^ b[38] ^ b[40] ^ b[42] ^ b[21] ^ b[27] ^ b[31] ^ b[35] ^ b[39] ^ b[41] ^ b[43] ^ b[22] ^ b[28] ^ b[36] ^ b[23] ^ b[29] ^ b[37] ^ b[44] ^ b[45] ^ b[46] ^ b[47] ^ 3 ^ b[49] ^ b[50] ^ b[24] ^ b[25] ^ b[52] ^ b[53] ^ b[54] ^ b[55] ^ b[57] ^ b[58] ^ b[59] ^ b[60] ^ b[65] ^ b[66] ^ b[70] ^ b[71];
    let bb = [44, b[20], 6241 & 255, b[26], b[30], b[34], 6383 & 255, b[38], b[40], (6241 >> 16) & 255, b[42], b[21], b[27], (6241 >> 8) & 255, 6241 & 255, b[31], b[35], 6383 & 255, b[39], b[41], b[43], b[22], b[28], b[32], (6383 >> 24) & 255, b[36], b[23], b[29], b[33], b[37], b[44], b[45], (6383 >> 16) & 255, b[46], b[47], 3, b[49], b[50], b[24], b[25], b[65], b[66], 0 & 255, (0 >> 8) & 255];
    bb = bb.concat(window_env_list).concat(checksum);
    let random_str = [].concat(gener_random(Math.random() * 10000, [3, 45]), gener_random(Math.random() * 10000, [1, 0]), gener_random(Math.random() * 10000, [1, 5])).map(c => String.fromCharCode(c)).join('');
    let result_str = random_str + rc4_encrypt(String.fromCharCode.apply(null, bb), String.fromCharCode.apply(null, [121]));
...
export class DouyinParser extends plugin {
    ...
    async parse(e) {
        ...
        try {
            ...
            const apiUrl = DY_API_URL.replace('{}', videoId);
