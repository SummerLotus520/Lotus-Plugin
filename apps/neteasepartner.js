...
const dataDir = path.join(lotusRoot, 'data', 'netease');
const logDir = path.join(lotusRoot, 'data', 'logs');
const lastRunLogPath = path.join(dataDir, 'lastRun-nep.log');
...
export class neteasePartner extends plugin {
    ...
    weapi(obj) {
        ...
        const rsaEncrypt = (key, pubKey, mod) => {
            ...
            const res = (keyBig ** BigInt('0x' + pubKey)) % BigInt('0x' + mod);
    ...
    runStartupSequence() {
        ...
        if (lastRunDate !== today) {
            ...
            if (now > scheduledToday) {
                logger.info('[网易云合伙人] 检测到今日任务错过，正在执行开机补签...');
    ...
    async partnerLogin(e) {
        ...
        if (cookie) await e.reply("登录成功，Cookie已缓存。");
        else await e.reply("验证失败，请确认手机号密码或API服务状态。");
    }

    async getPartnerLog(e) {
        const files = fs.readdirSync(logDir).filter(f => f.startsWith('nep-')).sort().reverse();
        if (files.length === 0) return e.reply("目录下暂无合伙人执行日志。");
        
        const latestContent = fs.readFileSync(path.join(logDir, files[0]), 'utf8');
